...
\end{equation}
Again, since no current enters an OP AMP and $V_i=0$, then $V_{+}=V_{-}=0$  and we are left with a capacitor $C_2$, which is a series of 2 capacitors $C_{21}$ and $C_{22}$, in parallel with a series of resistors $R_2$, $Z_{OPAMP}=0. T
